...
const EditPatientProfile = () => {
  ...
  const [name, setName] = useState(data.name || '');
  const [gender, setGender] = useState(data.gender || '');
  const [birthdate, setBirthdate] = useState(data.birthdate ? data.birthdate.split('T')[0] : '');
  const [email, setEmail] = useState(data.email || '');
  const [phone, setPhone] = useState(data.phone || '');
  const [idCard, setIdCard] = useState(data.IDcard || '');
  const [allergy, setAllergy] = useState(data.allergy || '');
  const [bloodType, setBloodType] = useState(data.bloodType || '');
  const [relative, setRelative] = useState(relativeData.name || '');
  const [relativeType, setRelativeType] = useState(relativeData.relativeType || '');
  const [relativePhone, setRelativePhone] = useState(relativeData.phoneNumber || '');
  const [currentAddress, setCurrentAddress] = useState(data.currentAddress || '');
  const [error, setError] = useState(false); // error message
